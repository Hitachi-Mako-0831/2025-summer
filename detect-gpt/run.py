...
import tqdm
import random
from sklearn.metrics import roc_curve, precision_recall_curve, auc, f1_score, precision_recall_curve
import argparse
import datetime
import os
import json
...
def replace_masks(texts):
    ...
    tokens = mask_tokenizer(texts, return_tensors="pt", padding=True, truncation=True, max_length=512).to(DEVICE)
    outputs = mask_model.generate(**tokens, max_length=150, do_sample=True, top_p=args.mask_top_p, num_return_sequences=1, eos_token_id=stop_id)
    return mask_tokenizer.batch_decode(outputs, skip_special_tokens=False)

# def replace_masks(texts):
#     # 1. 计算期望的掩码数量，用于确定停止ID
#     # stop_id 应该基于所有文本的最大掩码数量来计算，确保一次性确定
#     n_expected = count_masks(texts)
#     if isinstance(n_expected, list): # 如果 count_masks 返回的是列表
#         max_n_expected = max(n_expected) if n_expected else 0
#     else: # 如果 count_masks 返回的是单个数值
#         max_n_expected = n_expected if n_expected > 0 else 0

#     if max_n_expected == 0:
#         # 如果没有掩码，使用分词器的 EOS ID 作为默认停止符，或者根据实际情况处理
#         stop_id = mask_tokenizer.eos_token_id
#     else:
...
#     # 2. **核心修改：引入批处理大小**
#     # 这是一个非常关键的参数，你需要根据你的GPU显存来调整。
#     # 对于 RTX 3090 和 T5-3B 这样的组合，从 1 或 2 开始尝试是安全的。
#     # 你可以把它作为命令行参数传递 (例如 --mask_batch_size)，
#     # 如果没有设置，这里给一个默认值。
#     mask_batch_size = getattr(args, 'mask_batch_size', 2) # 建议从2开始尝试

#     # 3. **核心修改：对文本列表进行分批处理**
#     for i in tqdm.tqdm(range(0, len(texts), mask_batch_size), desc="Replacing masks in batches"):
#         batch_texts = texts[i : i + mask_batch_size]
...
#             truncation=True,       # **启用截断**：解决 (599 > 512) 问题
#             max_length=mask_tokenizer.model_max_length # **使用模型最大长度**
#         ).to(DEVICE)

#         # 5. 调用 mask_model.generate 进行文本生成
#         outputs = mask_model.generate(
#             **tokens,
#             max_length=150, # 这是生成文本的长度，不是输入长度
#             do_sample=True,
...
#         # 6. 解码生成的文本并收集结果
#         fills_batch = mask_tokenizer.batch_decode(outputs, skip_special_tokens=False)
#         all_raw_fills.extend(fills_batch)
...
def extract_fills(texts):
    extracted_fills = []
    for x in texts:
        # 1. Clean the T5 output: remove <pad> from beginning, and all </s> at the end.
        # This gives us a cleaner string to work with.
        cleaned_text = x.replace("<pad>", "").strip()
        cleaned_text = re.sub(r'</s>+$', '', cleaned_text).strip() # Removes one or more </s> from end

        # 2. Extract fills: T5 generates content between <extra_id_N> tokens.
        # We need to find all substrings that are *between* these special tokens.
        # Example: "<extra_id_0> fill_0 <extra_id_1> fill_1 <extra_id_2>"
        # We want "fill_0", "fill_1"
        
        # Use a regex to find everything between <extra_id_N> and the next <extra_id_M> or end of string.
        # The pattern captures the content (.*?) that follows an <extra_id_N> token,
        # stopping just before another <extra_id_M> or the end of the string (\Z).
        # We also use re.IGNORECASE in case of unexpected casing of extra_id
        
        # Find all parts that look like an <extra_id_X> followed by some content.
        # T5 typically generates <extra_id_0> then the fill, then <extra_id_1> etc.
        # So we split by the extra_id tokens and take the non-empty parts.
        
        parts = re.split(r"<extra_id_\d+>", cleaned_text)
        
        current_text_fills = []
        # The first part (parts[0]) is usually what came *before* <extra_id_0>, which we don't need for fills.
        # The subsequent parts (parts[1:]) are the actual fills.
        # If the input was "<extra_id_0>FILL_0<extra_id_1>FILL_1", parts would be ['', 'FILL_0', 'FILL_1']
        for part in parts[1:]: # Start from index 1 to get the actual fills
            fill_content = part.strip()
            if fill_content: # Only add if not empty (e.g., if two <extra_id> were adjacent)
                current_text_fills.append(fill_content)
        
        extracted_fills.append(current_text_fills)
...
def apply_extracted_fills(masked_texts, extracted_fills):
    ...
    # Ensure inputs are correctly paired
    if len(masked_texts) != len(extracted_fills):
        print(f"ERROR: Mismatched lengths in apply_extracted_fills: masked_texts={len(masked_texts)}, extracted_fills={len(extracted_fills)}")
        return [""] * len(masked_texts) # Return empty strings to propagate error

    for idx, masked_text in enumerate(masked_texts):
        current_fills = extracted_fills[idx] # This will be a list of strings
        current_perturbed_text = masked_text # Start with the original masked text

        # Get the number of masks we expect in this original masked text
        num_expected_masks = masked_text.count("<extra_id_")

        # Replace each mask token with the corresponding fill
        for fill_idx in range(num_expected_masks):
            placeholder = f"<extra_id_{fill_idx}>" # E.g., <extra_id_0>, <extra_id_1>
            
            if placeholder in current_perturbed_text:
                if fill_idx < len(current_fills):
                    # Replace with the extracted fill
                    current_perturbed_text = current_perturbed_text.replace(placeholder, current_fills[fill_idx], 1)
                else:
                    # If we don't have enough fills for all masks, replace remaining with empty string
                    current_perturbed_text = current_perturbed_text.replace(placeholder, "", 1)
                    # We don't print a warning here, as the outer loop handles 'no fills'
            else:
                # This mask wasn't found in the text, indicating a possible logic error
                # Or the text was already partially filled in a previous attempt
                pass # Don't do anything if the placeholder is already gone
        
        # Final check: If any mask tokens are still present, it means filling failed for this text.
        # Mark it as an empty string to trigger the outer retry logic.
        if re.search(r"<extra_id_\d+>", current_perturbed_text):
            perturbed_texts.append("") # This text effectively "has no fills"
        else:
            # If all masks are replaced, clean up any extra whitespace and add to result
            perturbed_texts.append(current_perturbed_text.strip())
...
def get_ll(text):
    if args.openai_model:
        # OpenAI 模型部分保持不变，因为API调用内部处理了长度限制
        kwargs = { "engine": args.openai_model, "temperature": 0, "max_tokens": 0, "echo": True, "logprobs": 0}
        r = openai.Completion.create(prompt=f"<|endoftext|>{text}", **kwargs)
        result = r['choices'][0]
        tokens, logprobs = result["logprobs"]["tokens"][1:], result["logprobs"]["token_logprobs"][1:]
        ...
        with torch.no_grad():
            # --- 关键修改在这里 ---
            # 1. 启用 truncation=True
            # 2. 指定 max_length。最安全的方式是使用 base_tokenizer.model_max_length
            #    或者如果你确定模型最大长度是1024，也可以直接写 1024。
            #    同时确保 padding=True，因为模型通常期望批次内的序列长度一致。
            tokenized = base_tokenizer(
                text,
                return_tensors="pt",
                padding=True,          # 确保批次内的序列被填充到相同长度
                truncation=True,       # <--- 启用截断，这是解决问题的关键！
                max_length=512 # <--- 使用分词器的最大长度
            ).to(DEVICE)

            labels = tokenized.input_ids.clone() # 建议使用 .clone() 避免潜在的副作用

            # 对于因果语言模型，损失计算通常是预测下一个词的对数似然。
            # Transformers 库在 labels=input_ids 时会正确处理内部的 shift_labels 操作。
            return -base_model(**tokenized, labels=labels).loss.item()
...
def eval_supervised(data, model):
 ...
 # --- 新增 F1 分数计算 ---
    # 为了计算F1分数，我们需要将概率预测转换为离散的类别预测（0或1）。
    # 通常会选择一个阈值，比如0.5，或者根据Precision-Recall曲线找到最佳F1分数对应的阈值。
    # 这里我们使用一个简单的0.5阈值作为示例，也可以计算所有可能阈值下的最大F1。

    # 合并所有预测概率和真实标签
    # 假设 'real' 文本的标签是 1 (正类)，'fake' 文本的标签是 0 (负类)
    all_preds = real_preds + fake_preds
    all_labels = [1] * len(real_preds) + [0] * len(fake_preds) # 真实标签列表

    # 计算在不同阈值下的精确率、召回率和F1分数，并找到最佳F1分数
    precisions, recalls, thresholds = precision_recall_curve(all_labels, all_preds)
    f1_scores = []
    # 遍历所有可能的阈值，计算F1分数
    # 注意：thresholds比precisions/recalls少一个，因为最后一个召回率对应的阈值是1.0
    for i in range(len(precisions)):
        if (precisions[i] + recalls[i]) == 0: # 避免除以零
    ...
    if f1_scores: # 确保 f1_scores 不为空
        best_f1 = max(f1_scores)
        # 找到对应最佳F1分数的索引
        best_f1_idx = f1_scores.index(best_f1)
        # 确保 best_f1_idx 在 thresholds 的有效范围内
        if best_f1_idx < len(thresholds):
            best_f1_threshold = thresholds[best_f1_idx]
        else: # 如果 best_f1_idx 指向 precisions/recalls 的最后一个，其阈值通常是1.0
            best_f1_threshold = 1.0 
    
    print(f"{model} Best F1 Score: {best_f1:.4f} (at threshold: {best_f1_threshold:.4f})")
    # --- F1 分数计算结束 ---
    ...
    return {
        ...
        'pr_metrics': {
            ...
            'precision': p,
            'recall': r,
            'best_f1': best_f1, # 添加最佳F1分数
            'best_f1_threshold': best_f1_threshold, # 添加最佳F1分数对应的阈值
        },
        'loss': 1 - pr_auc, # 保持不变
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--only_perturbation', action='store_true', help='Only run perturbation experiments, skip baselines.')
    ...
    if not args.skip_baselines and not args.only_perturbation:
